...
void getSDCData(char dataString[], char dataSize){
    char check;
...
void setSDCWords(char dataString[]){
    char check;
    printf("size:%d\n", getSendBufferSize(dataString));
    check = ASA_SDC00_put(SDC_ID, 0, getSendBufferSize(dataString), dataString);
    if(check != 0) {printf("Write fail ...%d\n", check);}
    
    initBuffer(dataString);
}
...
int getSendBufferSize(char dataString[]){
    int counter = 0;
